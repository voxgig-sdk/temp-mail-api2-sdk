...
```lua
-- Create
local created, err = client:TemporaryEmail():create({ name = "Example" })
if err then error(err) end
...
Check `err` first (it is non-`nil` on failure), then use `value`:
    ...
    local temporary_email, err = client:TemporaryEmail():load({ id = "example_id" })
    if err then error(err) end
...
```lua
local temporary_email, err = client:TemporaryEmail():load({ id = "temporary_email_id" })
```
