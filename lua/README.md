...
It exposes the API as capitalised, semantic **Entities** — e.g. `client:TemporaryEmail()` — each with the same small set of operations (`load`, `create`, `remove`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/temp-mail-api2-sdk/releases)),
...
Create an instance: `local temporary_email = client:TemporaryEmail(nil)`
...
```lua
local temporary_email, err = client:TemporaryEmail():create({
})
```
...
```
lua/
├── temp-mail-api2_sdk.lua    -- Main SDK module
...
The main module (`temp-mail-api2_sdk`) exports the SDK constructor
