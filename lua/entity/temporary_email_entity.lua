...
local TemporaryEmailEntity = {}
TemporaryEmailEntity.__index = TemporaryEmailEntity


function TemporaryEmailEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TemporaryEmailEntity)
  self._name = "temporary_email"
  self._client = client
...
function TemporaryEmailEntity:get_name()
  return self._name
end


function TemporaryEmailEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TemporaryEmailEntity.new(self._client, opts)
end
...
function TemporaryEmailEntity:mark_deleted()
  self._deleted = true
end


function TemporaryEmailEntity:deleted()
  return true == self._deleted
end


function TemporaryEmailEntity:data_set(args)
  if args ~= nil then
...
function TemporaryEmailEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TemporaryEmailEntity:match_set(args)
  if args ~= nil then
...
function TemporaryEmailEntity:match_get()
...
function TemporaryEmailEntity:stream(action, args, callopts)
...
function TemporaryEmailEntity:_run_op(ctx, post_done)
  local utility = self._utility
