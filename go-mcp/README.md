...
two agent tools — `temp-mail-api2_list` and `temp-mail-api2_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/temp-mail-api2-mcp)
make build

# 2. Provide credentials via the environment
export TEMP_MAIL_API2_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user temp-mail-api2 \
  -- /absolute/path/to/temp-mail-api2-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./temp-mail-api2-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// temp-mail-api2_list: first page of records
{ "entity": "temporary_email" }
{ "entity": "temporary_email", "query": { } }

// temp-mail-api2_load: one record by id
{ "entity": "temporary_email", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/temp-mail-api2-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user temp-mail-api2 \
     -- "$PWD"/dist/*/temp-mail-api2-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `temp-mail-api2_list` and `temp-mail-api2_load` tools now appear
   in new sessions. Ask the agent to *"list temporary_email using temp-mail-api2"*
   and it calls `temp-mail-api2_list` with `{"entity":"temporary_email"}`.
...
```sh
export TEMP_MAIL_API2_APIKEY=sk_live_xxx            # API key
export TEMP_MAIL_API2_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `temp-mail-api2_list` tool
...
### Call the `temp-mail-api2_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `temp-mail-api2_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `temp-mail-api2_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TEMP_MAIL_API2_APIKEY` | API key sent with every request. |
| `TEMP_MAIL_API2_BASE` | Optional override of the API base URL. |
...
```sh
./temp-mail-api2-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"temp-mail-api2_load","arguments":{"entity":"temporary_email","query":{"id":1}}}}'
```
