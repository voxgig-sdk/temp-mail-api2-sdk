...
class TemporaryEmailDirectTest < Minitest::Test
  def test_direct_load_temporary_email
    setup = temporary_email_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-temporary_email", setup[:live] ? "live" : "unit")
    if _should_skip
...
def temporary_email_direct_setup(mockres)
