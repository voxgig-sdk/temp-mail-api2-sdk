...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/temp-mail-api2-cli)
make build

# 2. See usage (words, entities, env vars)
./temp-mail-api2-cli --help

# 3. Provide credentials once, via the environment
export TEMP_MAIL_API2_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./temp-mail-api2-cli load 1 temporary_email            # {id:1} shorthand
./temp-mail-api2-cli load '{id:1}' temporary_email       # explicit match map

# 5. Override the API base URL for a single call
TEMP_MAIL_API2_BASE=https://api.example.com ./temp-mail-api2-cli load 1 temporary_email

# 6. No arguments -> interactive REPL
./temp-mail-api2-cli
temp-mail-api2> load 1 temporary_email
temp-mail-api2> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/temp-mail-api2-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/temp-mail-api2-cli load 1 temporary_email
   ```
...
```sh
./temp-mail-api2-cli load 1 temporary_email          # scalar shorthand for {id:1}
./temp-mail-api2-cli load '{id:1}' temporary_email     # explicit match map
```
...
./temp-mail-api2-cli load 1 temporary_email
```
...
Run with no arguments to open a REPL (prompt `temp-mail-api2>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `temporary_email`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/temp-mail-api2-cli`. |
...
(`./temp-mail-api2-cli <expr>`) and interactively (the REPL), and expressions compose the
