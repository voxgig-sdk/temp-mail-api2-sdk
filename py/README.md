...
```python
result, err = client.TemporaryEmail(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
```python
# Create
created, _ = client.TemporaryEmail(None).create({"name": "Example"}, None)

# Remove
client.TemporaryEmail(None).remove({"id": created["id"]}, None)
```
