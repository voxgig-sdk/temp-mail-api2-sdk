...
class TemporaryEmailEntity extends TempMailApi2EntityBase<TemporaryEmail> {

  constructor(client: TempMailApi2SDK, entopts: any) {
    super(client, entopts)
    this.name = 'temporary_email'
    this.name_ = 'temporary_email'
    this.Name = 'TemporaryEmail'
  }


  make(this: TemporaryEmailEntity) {
    return new TemporaryEmailEntity(this._client, this.entopts())
  }



  async load(this: any, reqmatch?: TemporaryEmailLoadMatch, ctrl?: Control): Promise<TemporaryEmailEntity> {
    ...
    try {
      ...
      if (err) {
        ...
        // Promise<TemporaryEmail> return stays clean under strict null checks.
  ...
  async create(this: any, reqdata?: TemporaryEmailCreateData, ctrl?: Control): Promise<TemporaryEmailEntity> {
  ...
  async remove(
    this: any, reqmatch?: TemporaryEmailRemoveMatch, ctrl?: Control,
  ): Promise<TemporaryEmailEntity> {
    ...
    try {
      ...
      if (err) {
        ...
        // Off-happy-path (throw disabled): typed as any so the method's
        // Promise<TemporaryEmailEntity> return stays clean under strict null checks.
        return undefined as any
      }
    }
  }
