...
(0, node_test_1.describe)('TemporaryEmailDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-temporary_email', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-temporary_email', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.email = "user123@tempmail.boomlify.com";
        }
        else {
            params.email = 'direct01';
        }
        const result = await client.direct({
            path: 'temp-mail/{email}/inbox',
            method: 'GET',
            params,
            query,
        });
...
function liveScenariosActive() { return false && process.env.TEMP_MAIL_API2_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['TEMP_MAIL_API2_TEST_TEMPORARY_EMAIL_ENTID'];
...
//# sourceMappingURL=TemporaryEmailDirect.test.js.map