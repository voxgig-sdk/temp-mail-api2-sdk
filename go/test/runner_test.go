...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("TEMP_MAIL_API2_TEST_LIVE") == "TRUE" ||
		os.Getenv("TEMP_MAIL_API2_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("TEMP_MAIL_API2_TEST_EXPLAIN"); explain != "" {
		m["TEMP_MAIL_API2_TEST_EXPLAIN"] = explain
	}
